...
function MetricRow({ label, naive, ml, format: fmt = (v: number) => v.toFixed(2), higherIsBetter = true }: {
  ...
  return (
    <tr className="border-b border-[#1f1f1f]/50 hover:bg-[#0a0a0a] transition-colors">
      <td className="px-3 py-2.5 text-[11px] text-[#9a9a9a] font-mono">{label}</td>
      <td className="px-3 py-2.5 text-[11px] text-[#e8e8e8] font-mono tabular-nums text-right">{fmt(naive)}</td>
      <td className="px-3 py-2.5 text-right">
        <span className={clsx(
          'px-2 py-0.5 rounded text-[11px] font-mono tabular-nums font-semibold',
          mlBetter ? 'text-[#00ff66] bg-[#00ff66]/10' : 'text-[#e8e8e8]'
        )}>
          {fmt(ml)}
...
const MONTH_COLORS = (v: number) =>
  v > 10 ? '#00ff66' : v > 3 ? '#00d4ff' : v > 0 ? '#4a9f7a' :
  v > -3 ? '#ffcc00' : v > -10 ? '#ff7043' : '#ff3366'
...
export default function Backtest() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-lg font-semibold text-[#e8e8e8] font-mono">Backtest</h1>
      ...
      <div className="bg-[#111111] border border-[#1f1f1f] rounded-lg p-4">
        <div className="flex flex-wrap items-end gap-4">
          <div className="space-y-1">
            <label className="text-[9px] text-[#555555] font-mono uppercase tracking-wider">Start Date</label>
            <input
              type="date"
              value={startDate}
              onChange={(e) => setStartDate(e.target.value)}
              className="bg-[#000000] border border-[#1f1f1f] rounded px-3 py-1.5 text-[11px] font-mono text-[#e8e8e8] focus:outline-none focus:border-[#00d4ff] transition-colors"
            />
          </div>
          <div className="space-y-1">
            <label className="text-[9px] text-[#555555] font-mono uppercase tracking-wider">End Date</label>
            <input
              type="date"
              value={endDate}
          ...
          <div className="space-y-1">
            <label className="text-[9px] text-[#555555] font-mono uppercase tracking-wider">Strategy</label>
            <select
              value={strategy}
          ...
          <button
            onClick={handleRun}
            disabled={loading}
            className="flex items-center gap-2 px-5 py-1.5 bg-[#00d4ff]/10 border border-[#00d4ff]/40 rounded text-[11px] font-mono text-[#00d4ff] hover:bg-[#00d4ff]/20 transition-all disabled:opacity-50 font-semibold"
          >
        ...
        {error && (
          <div className="flex items-center gap-2 mt-3 text-[10px] text-[#ffcc00] font-mono">
        ...
        <>
          ...
          <div className="bg-[#111111] border border-[#1f1f1f] rounded-lg overflow-hidden">
            <div className="px-4 py-3 border-b border-[#1f1f1f] bg-[#0a0a0a] flex items-center justify-between">
              <h2 className="text-[10px] font-mono text-[#555555] uppercase tracking-wider">Strategy Comparison</h2>
              <div className="flex items-center gap-4 text-[10px] font-mono">
                <span className="flex items-center gap-1.5 text-[#9a9a9a]">
                  <span className="w-2.5 h-2.5 rounded bg-[#9a9a9a]" /> Naive
                </span>
                <span className="flex items-center gap-1.5 text-[#00ff66]">
                  <span className="w-2.5 h-2.5 rounded bg-[#00ff66]" /> ML
                </span>
              </div>
            </div>
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1f1f1f]">
                  <th className="px-3 py-2 text-left text-[9px] text-[#555555] uppercase font-medium font-mono">Metric</th>
                  <th className="px-3 py-2 text-right text-[9px] text-[#555555] uppercase font-medium font-mono">Naive</th>
                  <th className="px-3 py-2 text-right text-[9px] text-[#555555] uppercase font-medium font-mono">ML</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div className="bg-[#111111] border border-[#1f1f1f] rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[10px] font-mono text-[#555555] uppercase tracking-wider">
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      activeView === v
                        ...
                        : 'text-[#555555] hover:text-[#9a9a9a] border border-transparent'
                    )}
                  >
                ...
                <AreaChart data={equityData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="mlGrad" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <stop offset="95%" stopColor="#00ff66" stopOpacity={0.02} />
                    </linearGradient>
                    <linearGradient id="naiveGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#9a9a9a" stopOpacity={0.2} />
                      <stop offset="95%" stopColor="#9a9a9a" stopOpacity={0.02} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1f1f1f" />
                  <XAxis dataKey="time" tick={{ fill: '#555555', fontSize: 9, fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={{ stroke: '#1f1f1f' }} interval={4} />
                  <YAxis tick={{ fill: '#555555', fontSize: 9, fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={false} tickFormatter={v => `€${v}`} width={52} />
                  <Tooltip
                  ...
                  <Area type="monotone" dataKey="naive" stroke="#9a9a9a" strokeWidth={1.5} fill="url(#naiveGrad)" dot={false} />
                  <Area type="monotone" dataKey="ml" stroke="#00ff66" strokeWidth={2} fill="url(#mlGrad)" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              /* Monthly heatmap */
              <div className="space-y-3">
                <div className="text-[9px] text-[#555555] font-mono uppercase">Naive Strategy</div>
                <div className="grid grid-cols-6 gap-1">
                  {naiveMonthly.map((m) => (
                    <div key={m.month} className="rounded p-2 text-center" style={{ backgroundColor: `${MONTH_COLORS(m.return_pct)}20`, border: `1px solid ${MONTH_COLORS(m.return_pct)}40` }}>
                      <div className="text-[9px] text-[#555555] font-mono">{m.month}</div>
                ...
                <div className="text-[9px] text-[#555555] font-mono uppercase mt-2">ML Strategy</div>
                <div className="grid grid-cols-6 gap-1">
                  {mlMonthly.map((m) => (
                    <div key={m.month} className="rounded p-2 text-center" style={{ backgroundColor: `${MONTH_COLORS(m.return_pct)}20`, border: `1px solid ${MONTH_COLORS(m.return_pct)}40` }}>
                      <div className="text-[9px] text-[#555555] font-mono">{m.month}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Win rate */}
            <div className="bg-[#111111] border border-[#1f1f1f] rounded-lg p-4">
              <h3 className="text-[10px] font-mono text-[#555555] uppercase tracking-wider mb-4">Win Rate</h3>
              <div className="space-y-3">
                {[
                  { label: 'Naive', value: result.naive.win_rate, color: '#9a9a9a' },
                  { label: 'ML', value: result.ml.win_rate, color: '#00ff66' }
                ].map(({ label, value, color }) => (
                  <div key={label}>
                    <div className="flex justify-between text-[10px] font-mono mb-1">
                      <span style={{ color }}>{label}</span>
                      <span className="text-[#e8e8e8]">{(value * 100).toFixed(1)}%</span>
                    </div>
              ...
              <div className="mt-3 text-[9px] text-[#555555] font-mono">
            ...
            {/* Profit factor */}
            <div className="bg-[#111111] border border-[#1f1f1f] rounded-lg p-4">
              <h3 className="text-[10px] font-mono text-[#555555] uppercase tracking-wider mb-4">Profit Factor</h3>
              <div className="space-y-3">
                {[
                  { label: 'Naive', value: result.naive.profit_factor, max: 3, color: '#9a9a9a' },
                  { label: 'ML', value: result.ml.profit_factor, max: 3, color: '#00ff66' }
                ].map(({ label, value, max, color }) => (
                  <div key={label}>
                    <div className="flex justify-between text-[10px] font-mono mb-1">
                      <span style={{ color }}>{label}</span>
                      <span className="text-[#e8e8e8]">{value.toFixed(2)}x</span>
                    </div>
                    <div className="h-2.5 bg-[#1f1f1f] rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-700"
      ...
      {!result && !loading && (
        <div className="bg-[#111111] border border-[#1f1f1f] rounded-lg p-12 text-center">
          <Play size={24} className="text-[#1f1f1f] mx-auto mb-3" />
